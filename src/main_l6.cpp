...
int I2C_RegisterRead(uint8_t addr, uint8_t reg_id, uint8_t *data_out, uint8_t data_len) {
    ...
    for (int i = 0; i < data_len; i++) {
        ...
        data_out[i] = I2C_read(i < data_len - 2);
...
static int32_t BMP280_compensation_p_32(int32_t raw_value, BMP280_compensation_p *compensation);
...
BMP280_compensation_p compensation_p;
...
void loop() {
    ...
    if (!compensation_p.dig_P1) {
        uint8_t raw_compensation[6];
        res = I2C_RegisterRead(address, 0x88, raw_compensation, 6);
        _I2C_TRANSACTION_CHECK(res, "reading compensation registers");
        // convert the raw compensation values to 16-bit integers (Little Endian)
        // note: only T1 is unsigned, the others have signs (see datasheet)
        compensation_p.dig_P1 = ((uint16_t) raw_compensation[0] | ((uint16_t) raw_compensation[1] << 8));
        compensation_p.dig_P2 = (int16_t) ((uint16_t) raw_compensation[2] | ((uint16_t) raw_compensation[3] << 8));
        compensation_p.dig_P3 = (int16_t) ((uint16_t) raw_compensation[4] | ((uint16_t) raw_compensation[5] << 8));
        compensation_p.dig_P4 = (int16_t) ((uint16_t) raw_compensation[6] | ((uint16_t) raw_compensation[7] << 8));
        compensation_p.dig_P5 = (int16_t) ((uint16_t) raw_compensation[8] | ((uint16_t) raw_compensation[9] << 8));
        compensation_p.dig_P6 = (int16_t) ((uint16_t) raw_compensation[10] | ((uint16_t) raw_compensation[11] << 8));
        compensation_p.dig_P7 = (int16_t) ((uint16_t) raw_compensation[12] | ((uint16_t) raw_compensation[13] << 8));
        compensation_p.dig_P8 = (int16_t) ((uint16_t) raw_compensation[14] | ((uint16_t) raw_compensation[15] << 8));
        compensation_p.dig_P9 = (int16_t) ((uint16_t) raw_compensation[16] | ((uint16_t) raw_compensation[17] << 8));
        Serial.println("Compensation parameters:");
        Serial.println(compensation_p.dig_P1);
        Serial.println(compensation_p.dig_P2);
        Serial.println(compensation_p.dig_P3);
        Serial.println(compensation_p.dig_P4);
        Serial.println(compensation_p.dig_P5);
        Serial.println(compensation_p.dig_P6);
        Serial.println(compensation_p.dig_P7);
        Serial.println(compensation_p.dig_P8);
        Serial.println(compensation_p.dig_P9);
    }
...
int32_t BMP280_CompensatePressure_32(int32_t raw_value, struct BMP280_compensation_p *compensation) {
    int32_t var1, var2, P;
    // this is the raw temperature compensation formula from datasheet
    var1 = (((int32_t) bmp280_t_fine) >> 1) - (int32_t) 64000;
    var2 = (((var1 >> 2) * (var1 >> 2)) >> 11) * ((int32_t) compensation->dig_P6);
    var2 = var2 + ((var1 * ((int32_t) compensation->dig_P5)) << 1);
    var1 = ((compensation->dig_P3) * var1 * var1 / 524288.0 + (compensation->dig_P2) * var1) / 524288.0;
    var1 = (1.0 + (var1 / 32768.0)) * ((int32_t) compensation->dig_P1);
    P = P + (var1 + var2 + ((int32_t) compensation->dig_P7)) / 16.0;
    return P;
}
